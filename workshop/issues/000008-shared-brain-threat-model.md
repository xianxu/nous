...
**actual_hours: 2** (M1+M2 closed 2026-05-05 ~1.5h focused; M3 closed 2026-05-07 ~0.45 hr focused per `active-time-v3.py`).

**Methodology note**: the M3 portion was originally a hand-guess that v2.1's session-wide mention-weighting inflated to 14.07 hr (multi-issue session over-attribution — see `brain/data/life/42shots/velocity/baseline-v3.md`). Re-run under v3 (`active-time-v3.py --commit-weight 1.0`): **0.45 hr** for the close commit's segment. M3 ratio: 1.13× of its 0.2–0.4 hr estimate. Issue total 2 hr stands as M1+M2 estimated-and-shipped on 2026-05-05 + 0.5 hr M3.
