...
**actual_hours: 2 (estimate)** — frontmatter value is a judgment guess, not script-derived. M1+M2 closed 2026-05-05 with the threat-model doc + cross-references (~1.5h focused). M3 closed 2026-05-07 with the wife conversation (~30 min) + this log entry. Within the original 0.7–1.8 hr focused estimate.

**Methodology note**: the canonical `xx-issues` `active-time.py` procedure returns 14.07 hr mention-weighted attribution to #8 across the commit window — wildly over-counted because the bulk of that window was an 8-hour brain session that touched 5+ issues simultaneously (`#3, #4, #8, #10, #11`). Mention-weighted attribution can't distinguish which issue drove the work in mixed-issue sessions. Future calibration data points should treat the script as authoritative only for sessions where one issue dominates (charon-launch-push baseline-v2.1 data points are the right shape; this session is not).
