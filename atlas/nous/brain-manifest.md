...
- `autosave: on | off` — optional, default on. The **commit** axis: whether the brainsync daemon auto-commits tracked-file edits (5s debounce) as a local safety net. See `autosave-and-checkpoint.md`.
- `publish: on | off` — optional, default *derived*. The **push** axis (`nous#47`): whether the daemon auto-pushes committed changes to origin. Absent → gcrypt/shared brains push, a private plain-remote brain does not; `on` → the "private but published" opt-in (auto-push a single-recipient brain on a plain remote); `off` → never auto-push (pull still runs). Orthogonal to `autosave:`
