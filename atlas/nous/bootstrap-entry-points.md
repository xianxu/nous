...
Interactive end-to-end. Prompts for GitHub owner+repo (creates the repo via `gh repo create --private`), GPG identity, target path. Performs `git init`, runs `nous/setup.sh --all --yes` to symlink `construct/`, authors `.brain/config.md`, configures the `gcrypt::ssh://...` remote, force-pushes the initial encrypted commit.
